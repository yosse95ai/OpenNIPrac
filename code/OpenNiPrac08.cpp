...
public:
    ...
    {
        // デバイスを取得
        openni::Status ret = device.open("recoder.oni");
        ...
        // ストリームデータを記録する
        //recoder.create("recoder.oni");
        //recoder.attach(colorStream);
        //recoder.attach(depthStream);
        //recoder.start();
    }
...
private:
    ...
    cv::Mat showColorStream(const openni::VideoFrameRef& colorFrame)
    {
        ...
        // Xtion IR Stream
        else if (colorFrame.getVideoMode().getPixelFormat() == openni::PIXEL_FORMAT_GRAY16) {
            // XtionのIRのフォーマットは16ビットグレースケール
            // 実際は255諧調らしく、CV_8Uに落とさないとえない
            colorImage = cv::Mat(colorFrame.getHeight(), colorFrame.getWidth(),
                CV_16UC1, (unsigned char*)colorFrame.getData());
            colorImage.convertTo(colorImage, CV_8U);
        }
        // Kinect for Windows IR Stream
        else {
            // KinectのIRのフォーマットは8ビットグレースケール
            // Kinect SDKは16ビットグレイスケール
            colorImage = cv::Mat(colorFrame.getHeight(), colorFrame.getWidth(),
